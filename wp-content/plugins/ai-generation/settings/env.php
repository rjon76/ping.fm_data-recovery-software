...
defined('OPENAI_API_KEY') || define('OPENAI_API_KEY', '');
$OPENAI_API_KEY = OPENAI_API_KEY;

defined('SCREENSHOT_KEY') || define('SCREENSHOT_KEY', '');
...
function getSoftUrlFromString($str) {
    preg_match_all('#<h2><a [h,H]ref=[*,",\'](?:https?)://[^\s\,]+[^>]*.(.*?)<\/a></h2>#i', $str, $matches);
    return $matches;
}

function replaceOfficialWebsite($str, $url) {
    if(preg_replace('#<a [h,H]ref=[*,",\']'.$url.'[^>]*.Official Website<\/a>#is', "", $str)) {
        return preg_replace('#<a [h,H]ref=[*,",\']'.$url.'[^>]*.Official Website<\/a>#is', "", $str);
    }
...
function replaceInvalidUrl($str, $url) {
    if(preg_replace('#<a [h,H]ref=[*,",\']'.$url.'+[^>]*.(.*?)<\/a>#is', "\$1", $str)) {
        return preg_replace('#<a [h,H]ref=[*,",\']'.$url.'+[^>]*.(.*?)<\/a>#is', "\$1", $str);
    }
...
if(!function_exists('fetch_headers')) {
...
function generateImgWithTitle($title, $image_src, $isAi = false, $lang = '', $title_original = '', $domainUrl = '', $image_name = '') {
    ...
    if(!$isScreenshot) {
        $save_file = $BASE_UPLOADS . '/ai/' .str_replace("--", "-", str_replace("---", "-", str_replace([" ", "?", '&', '.', ":", ";", "/", "‘", "’", "'"], "-", $title))).'.jpg';
    } else {
        $save_file = $BASE_UPLOADS . '/ai/' . str_replace(['/', '.', ':', '?', '='], '-', $image_name) . $lang . '.jpg';
    }
    ...
    if($lang === '') {
        ...
        if($isScreenshot) {
            $image = imagecreatetruecolor(400, 400);
            $white_alpha = imagecolorallocatealpha($image, 255, 255, 255, 0);

            imagefill($image, 0, 0, $white_alpha);
            imagecopyresampled($capture, $image, 0, round($height - 106), 0, 0, $width, 200, 400, 400);
            imagettftextcenter($capture, 16, 0, 20, round($height - 70), $black, $font_path, $domainUrl, $lang);
        }
        // 
        ...
        if(!$isScreenshot) {
            $save_file = $BASE_UPLOADS . '/ai/' .str_replace("--", "-", str_replace("---", "-", str_replace([" ", "?", '&', '.', ":", ";", "/", "‘", "’", "'"], "-", $title_original))). "-$lang" .'.jpg';
        } else {
            $save_file = $BASE_UPLOADS . '/ai/' . str_replace(['/', '.', ':', '?', '='], '-', $image_name) . $lang . '.jpg';
        }
        ...
        imagefill($image, 0, 0, $black);
        if($lang === 'Arabic' && $isScreenshot) {
            imagecopyresampled($capture, $image, $width / 4, round(($height - $hSq) / 2), 0, 0, round($width), $hSq, 400, 400);
        } else {
            imagecopyresampled($capture, $image, $width / 2.6, round(($height - $hSq) / 2), 0, 0, round($width - $width / 2.6), $hSq, 400, 400);
        }
...
function autoImport($array, $domain_url, $cron_job_key) {
    ...
    foreach($array as $id) {
        fetch_headers( "$domain_url/wp-load.php?import_key=$cron_job_key&import_id=$id&action=trigger" );
        sleep(10);
        fetch_headers( "$domain_url/wp-load.php?import_key=$cron_job_key&import_id=$id&action=processing" );
        sleep(5);
    }
}

function screenshotlayer($url, $args, $access_key) {
    
    // set secret keyword (defined in account dashboard)
    $secret_keyword = "cleverfiles";
